                ...
                return postRepository.findAliveByAuthor(target.getId()).stream()
                                .map(p -> new AdminPostResponse(
                                                ...
                                                (p.getTags() == null) ? List.of()
                                                                : p.getTags().stream().map(noa.entity.Tag::getName)
                                                                                .toList(),
        ...
        public List<AdminReportResponse> listReports(String status) {
                ...
                return reports.stream().map(r -> {
                        ...
                        if ("POST".equals(r.getTargetType())) {
                                // 投稿通報：対象が投稿そのもの
                                postId = r.getTargetId();
                                targetBody = postRepository.findById(r.getTargetId())
                                                .map(p -> p.isDeleted() ? "(削除済みの投稿)" : p.getBody())
                                                .orElse("(存在しない投稿)");
                        } else {
                                // コメント通報：親投稿idを取得
                                var commentOpt = commentRepository.findById(r.getTargetId());
                                postId = commentOpt.map(c -> c.getPost().getId()).orElse(null);
                                targetBody = commentOpt.map(c -> c.getBody()).orElse("(削除済みのコメント)");
                        }

                        return new AdminReportResponse(
                                        r.getId(), reporterHandle, r.getTargetType(), r.getTargetId(),
                                        postId, targetBody, r.getReason(), r.getStatus(), r.getCreatedAt());
                }).toList();
        }
